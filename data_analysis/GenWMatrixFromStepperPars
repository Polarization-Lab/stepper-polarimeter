function [W,amp] = GenWMatrixFromStepperPars(fn,LambdaList)
...
%Variables that do not depend on wavelength
    a_PSG=h5readatt(fn,'/cal/','PSG_retardance_slope');
    b_PSG=h5readatt(fn,'/cal/','PSG_retardance_intercept');
    a_PSA=h5readatt(fn,'/cal/','PSA_retardance_slope');
    b_PSA=h5readatt(fn,'/cal/','PSA_retardance_intercept');
    PSG_theta=h5readatt(fn,'/cal/','PSG_theta');
    PSA_theta=h5readatt(fn,'/cal/','PSA_theta');
    PSA_LP=h5readatt(fn,'/cal/','PSA_LP');
    amp=h5readatt(fn,'/cal/','Amp');
