...
%Starting fit guess
fitguess = mx;      %maximum value
%% Set fit input values
...
%Full set of PSG and PSA retardances from Spectral calibration
...
for n = 1:nLambda %across all wavelengths
    inputvec = [dg_rad(n), DelRad_g, da_rad(n), DelRad_a, LP_Rad, nSteps]; %Input values vector 
    for ii = 1:length(1:sqrt(PixelCount))                                  %across all pixel rows
        for jj = 1:length(1:sqrt(PixelCount))                              %across all pixel columns
            [fits,resnorm,res] = lsqcurvefit(func, fitguess, inputvec, meanfulldata(n,:,ii,jj),lb,ub); %Least-squares curve fit routine
            ampcaldata(n,ii,jj) = fits;                                    %Grab fitted amplitude terms
            RMSE_var(n,ii,jj,:) = std(res(:));                             %Grab RMSE
        end
    end
end
toc
...
clear M
for n = 1:nLambda
    Winv = pinv(squeeze(W(1,:,:))); %Create W^-1
...
for ii = 1:nLambda
    filename = ["test2_45" num2str(LambdaList(nLambda))];
